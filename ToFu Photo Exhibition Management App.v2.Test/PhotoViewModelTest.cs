...
namespace ToFuPhotoExhibitionManagementApp.v2.Test
{
	public class PhotoViewModelTest
	{
		...
		public async Task 読み込み後の値が正しい()
		{
			...
			viewModel.CategoryList.Count.Is(3);
			viewModel.SelectedCategory?.Name.Value.Is("Category1");
			viewModel.RoundList.Count.Is(3);
			viewModel.SelectedRound?.Name.Value.Is("Round1");
			viewModel.ManufacturerList.Count.Is(3);
			viewModel.SelectedManufacturer?.Name.Value.Is("Manufacturer1");
			viewModel.TeamList.Count.Is(3);
			viewModel.SelectedTeam?.Name.Value.Is("Team1");
			viewModel.CarList.Count.Is(3);
			viewModel.SelectedCar?.Name.Value.Is("Car1");
		}
	}
}
